...
namespace JobBars.Jobs {
    public static class DRG {
        ...
        public static CooldownConfig[] Cooldowns => new[] {
            new CooldownConfig($"{UIHelper.Localize(ActionIds.SecondWind)} ({UIHelper.Localize(JobIds.DRG)})", new CooldownProps {
                Icon = ActionIds.SecondWind,
                CD = 120,
                Triggers = new []{ new Item(ActionIds.SecondWind) }
            }),
            new CooldownConfig($"{UIHelper.Localize(ActionIds.LegSweep)} ({UIHelper.Localize(JobIds.DRG)})", new CooldownProps {
                Icon = ActionIds.LegSweep,
                Duration = 3,
                CD = 40,
                Triggers = new []{ new Item(ActionIds.LegSweep) }
            }),
            new CooldownConfig($"{UIHelper.Localize(ActionIds.Bloodbath)} ({UIHelper.Localize(JobIds.DRG)})", new CooldownProps {
            ...
            new CooldownConfig($"{UIHelper.Localize(ActionIds.ArmsLength)} ({UIHelper.Localize(JobIds.DRG)})", new CooldownProps {
